...
def simulate_future_pickups(future_days):
    ...
    last_date = pd.to_datetime(merged_cleaned['date'].iloc[-1])
...
def community_mapping():
    ...
    # Ensure geodata contains necessary columns
    required_columns = ['Latitude', 'Longitude', 'IncomeLevel', 'HamperRequests', 'Location']
    if not all(col in geodata.columns for col in required_columns):
        st.error(f"Missing one or more required columns in the community data: {required_columns}")
        return
